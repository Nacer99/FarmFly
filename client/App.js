const { BrowserRouter, Route, Switch, Link, useParams, useLocation, useHistory } = ReactRouterDOM;
...
function Home() {
  ...
  const handleFarmClick = (farmId) => {
    history.push(`/farm/${farmId}?date=${date.toISOString().split('T')[0]}`);
  };
...
function FarmDetails() {
  ...
  useEffect(() => {
    const fetchFarm = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API_BASE_URL}/api/farms/${id}`);
        setFarm(response.data);
        setError('');
      } catch (err) {
        setError('Error fetching farm details. Please try again.');
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  const handleBooking = async () => {
    try {
      const meals = Object.entries(selectedMeals).map(([mealId, quantity]) => ({mealId, quantity}));
    ...
    <div className="farm-details">
      ...
      {farm.meals.map(meal => (
        <div key={meal._id} className="meal-row">
          ...
          <input 
            ...
            min="0" 
            max={meal.quantity}
            value={selectedMeals[meal._id] || 0}
            onChange={(e) => handleMealSelection(meal._id, parseInt(e.target.value))}
          />
        </div>
      ))}
      <input 
      ...
      {bookingConfirmation && (
        <div className="booking-confirmation">
          ...
          <a href={bookingConfirmation.emailPreview} target="_blank" rel="noopener noreferrer">
    ...
    <BrowserRouter>
      <div className="App">
        ...
        <Switch>
          ...
          <Route path="/manager" component={FarmManagerDashboard} />
        </Switch>
      </div>
    </BrowserRouter>
  );
